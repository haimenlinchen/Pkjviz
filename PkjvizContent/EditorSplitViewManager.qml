...
Item {
    ...
    Component.onCompleted: {
        ...
        // 初始化默认标签页
        rootPane.addNewTab("文件1.py", "# 新文件1\n");
        rootPane.addNewTab("文件2.py", "# 新文件2\n");
        rootPane.addNewTab("文件3.py", "# 新文件3\n");
    }
    ...
    function handlePaneSplit(paneId, isVertical, childIds) {
        ...
        for (var j = 0; j < parentPane.childPanes.length; j++) {
            ...
            if (isVertical) {
                // 垂直分割时设置高度
                SplitView.preferredHeight = parentPane.height / 2;
            } else {
                // 水平分割时设置宽度
                SplitView.preferredWidth = parentPane.width / 2;
            }
        }
    }
