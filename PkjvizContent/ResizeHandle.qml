...
Rectangle {
    ...
    MouseArea {
        ...
        cursorShape: {
            if (isCorner) {
                if (root.anchors.right && root.anchors.bottom)
                    return Qt.SizeFDiagCursor;
                if (root.anchors.left && root.anchors.bottom)
        ...
        property int newSize: isDragging ? Math.max(minSize, Math.min(maxSize, isVertical ? -root.y : -root.x)) : currentSize
